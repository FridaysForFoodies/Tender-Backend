...
const exampleList: Example[] = [
    ...
    new Example(4, "Mad Max", "Et netus et malesuada fames ac turpis egestas maecenas."),
]
...
export class ExampleResolver {
    ...
    @Mutation(() => Boolean)
    async createCategory(@Arg("name") name: String, @Arg("description") description: String) {
        const lastItem = exampleList.reduce((previousValue, currentValue) => previousValue.id < currentValue.id ? currentValue : previousValue);
